...
export const resumeReview = async (req, res) => {
  try {
    ...
    const prompt = `
      You are an expert career coach. Analyze the following resume text and provide a structured review.
      ...
      5. Sample Rewrite of one weak bullet point

      Resume Content:
      ${pdfData.text}`;

    const response = await AI.chat.completions.create({
      model: "gemini-2.0-flash", // keep your model
      messages: [{ role: "user", content: prompt }],
      temperature: 0.7,
      max_tokens: 1000, // increased from 100 → 600
    });
